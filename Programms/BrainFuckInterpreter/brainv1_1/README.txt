...
Als Kommentar gilt alles, was kein BF Befehl ist und kein Debug Befehl (siehe "DEBUG MÖGLICHKEITEN).
...
DEBUG MÖGLICHKEIT:
--------------------------------
In BRAIN.COM v1.1 ist es nun möglich, den Code zu "debuggen".
Das dazu benötigte Zeichen ist die Raute (#).
Diese kann einfach irgendwo im BF Code angegeben werden.
Trifft der Interpreter auf dieses Zeichen, unterbricht er die Interpretation und kommuniziert mit dem Anwender.
Es können dann folgende EIngaben gemacht werden:
(Alle Eingaben sind klein zu schreiben)

Zeichen:|Wirkung:
c:	|Zeigt an, der wievielte Befehl ausgeführt wurde (unter Einbezug der Schleifen).
	|d.h. der Interpreter besitzt einen internen Counter, der immer, wenn ein Zeichen des BF Codes eingelesen
	|wird, um eins erhöht wird. Kommentare zählen auch mit!
------------
v:	|Zeigt den Inhalt der aktuellen Variable.
	|Kann dem BF Operator "." gleichgesetzt werden.
------------
i:	|Zeigt den Inhalt der aktuellen Variable als ASCII Code an.
	|Dies ist dann sinnvoll, wenn die Variable ein Zeichen beinhaltet, das entweder nicht darstellbar ist,
	|oder deren ASCII Wert nicht bekannt ist.
-------------
e:	|Beendet den Debug Modus und arbeitet den BF Code ganz normal wieder ab.