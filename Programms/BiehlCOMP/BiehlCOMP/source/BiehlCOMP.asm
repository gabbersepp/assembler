...
POP3_Connect PROC STDCALL hWnd:DWORD
  ...
  ADD ESI, 4              ;ESI = Pointer auf Variable für Socket Handle
  ADD EDI, 4
  XOR EDX, EDX
  MOV EDX, [EDI]
...
POP3_List PROC STDCALL hWnd:DWORD
  ...
  MOV ECX, 4
  MUL ECX
  ADD EAX, 4          ;Berechne benötigte Speichermenge
  
  ;Array sieht so aus: [EAX] = GRÖ?E_EMAIL_1
  ;                    [EAX+4] = GRÖßE_EMAIL_2
  ;                    ...
  ...
  MOV ECX, [EBX]
  PUSHA
  INVOKE GlobalFree, EBX        ;Struktur auflösen, da unnötig
  POPA
...
;; Version: 0.01                              ;;
;; Beschreibung: Löst ein durch Explode()     ;;
;; erzeugtes Array auf                        ;;
;; (c) 2005-2007 by Josef Biehler             ;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;  lptStrDest mit einer Länge von nCount                           ;;
